...
pub enum TaggedValue {
	...
	DistanceFunction(graphene_core::raster::DistanceFunction),
...
impl Hash for TaggedValue {
	fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
		...
		match self {
            ...
            Self::DistanceFunction(distance_function) => distance_function.hash(state),
...
impl<'a> TaggedValue {
	...
	pub fn to_any(self) -> Any<'a> {
		match self {
            ...
            TaggedValue::DistanceFunction(x) => Box::new(x),
	...
	pub fn ty(&self) -> Type {
		...
		match self {
            ...
            TaggedValue::DistanceFunction(_) => concrete!(graphene_core::raster::DistanceFunction),
	...
	pub fn try_from_any(input: Box<dyn DynAny<'a> + 'a>) -> Result<Self, String> {
		...
		match DynAny::type_id(input.as_ref()) {
			...
			x if x == TypeId::of::<graphene_core::raster::DistanceFunction>() => Ok(TaggedValue::DistanceFunction(*downcast(input).unwrap())),
